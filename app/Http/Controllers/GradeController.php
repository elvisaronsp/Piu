...
class GradeController extends Controller
{
	...
	public function dataChart(Request $request, $student_group_id, $unit_id){
		$school_id = Auth::user()->school_id;
		$result = Unit::join('grades', 'grades.unit_id', '=', 'units.id')
						->where([
							['grades.student_group_id', '=', $student_group_id],
							['units.id', '=', $unit_id]
						])
						->select(	
									'units.title as label',
								 	'units.id as unit_id',
									DB::raw('GROUP_CONCAT(grades.value) as notas')
								)
						->groupBy('grades.unit_id')
						->orderBy('units.title', 'desc')
						->get();
