...
export default {
  ...
  methods: {
    loadStudents() {
      axios
        .get(this.$routes.statistic.studentsCount)
        .then(response => (this.students.count = response.data.count))
        .catch(err =>
          this.showMessage(
            "Ops! Um erro ocorreu",
            "Não foi possível carregar a quantidade de alunos."
          )
        );
    },
    loadEmployeers() {
      axios
        .get(this.$routes.statistic.employeersCount)
        .then(response => (this.employeers.count = response.data.count))
        .catch(err =>
          this.showMessage(
            "Ops! Um erro ocorreu",
            "Não foi possível carregar a quantidade de funcionários."
          )
        );
    },
    loadGroups() {
      axios
        .get(this.$routes.statistic.groupsCount)
        .then(response => (this.groups.count = response.data.count))
        .catch(err =>
          this.showMessage(
            "Ops! Um erro ocorreu",
            "Não foi possível carregar a quantidade de turmas."
          )
        );
    }
  },
